    ...
    public class DisposeBlock : IDisposable
    {
        ...
        private static void OnDespawned(DisposeBlock that)
        {
            if (that._disposables != null)
            {
                ...
                for (var i = that._disposables.Count - 1; i >= 0; i--) that._disposables[i].Dispose();
        ...
        {
            if (_disposables == null) _disposables = ListPool<IDisposable>.Instance.Spawn();
        }
            ...
            where T : IDisposable
        {
            ...
            for (var i = 0; i < disposables.Count; i++) _disposables.Add(disposables[i]);
        }
        ...
        private void StoreSpawnedObject<T>(T obj, IDespawnableMemoryPool<T> pool)
        {
            ...
            else
            {
                ...
                if (_objectPoolPairs == null) _objectPoolPairs = ListPool<SpawnedObjectPoolPair>.Instance.Spawn();
