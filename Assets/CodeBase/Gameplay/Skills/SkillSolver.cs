...
namespace CodeBase.Gameplay.Skills
{
    public class SkillSolver : ISkillSolver, IInitializable
    {
        ...
        {
            var skill = Skill(heroAction);

            var hero = _heroRegistry.GetHero(heroAction.Caster.Id);
        ...
        {
            foreach (var skillApplier in _appliers)
                skillApplier.WarmUp();
        }
        ...
        public void SkillDelaysTick()
        {
            for (var i = _activeSkills.Count - 1; i >= 0; i--)
            {
                ...
                if (activeSkill.DelayLeft <= 0)
                {
                    ...
                    if (_heroRegistry.IsAlive(activeSkill.CasterId)) ApplySkill(activeSkill);
                }
            }
        }
        ...
        {
            var caster = _heroRegistry.GetHero(casterId);
        ...
        {
            return _staticDataService.HeroConfigFor(heroAction.Caster.TypeId)
