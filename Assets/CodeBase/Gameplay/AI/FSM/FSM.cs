...
namespace CodeBase.Gameplay.AI.FSM
{
    public class FSM : IArtificialIntelligence
    {
        private readonly IStaticDataService _staticDataService;
        private readonly IHeroRegistry _heroRegistry;

        public FSM(IStaticDataService staticDataService, IHeroRegistry heroRegistry)
        {
        ...
        private SkillKind ChooseSkillKind(IHero readyHero)
        {
            ...
            if (haveHeal)
            {
                foreach (var ally in _heroRegistry.AlliesOf(readyHero.Id))
                {
                    ...
                    if (allyBehaviour.State.HpPercentage < 0.3f)
                    {
            ...
            if (haveInitiativeBurn)
            {
                foreach (var enemy in _heroRegistry.EnemiesOf(readyHero.Id))
                {
                    ...
                    if (enemyBehaviour.State.InitiativePercentage > 0.8f)
                    {
